      ...
      <div class="container h-full px-6 py-24">
        <div
          ...
          <div class="mb-12 md:mb-0 md:w-8/12 lg:w-6/12">
            <img
                src="../assets/leftImageManBgR.png"
                class="w-full"
                alt="Phone image"/>
          </div>
          <div class="md:w-8/12 lg:ml-6 lg:w-5/12">
            <div class="mb-10">
              <ul class="text-sm font-medium text-center text-gray-500 divide-x divide-gray-200 rounded-lg flex
               shadow dark:divide-gray-700 dark:text-gray-400">
                <li v-for="(tab, index) in Object.keys(TabsEnum)" :key="tab" class="w-full">
                  <button @click="changeTab(index)"  :class="selectedTab === index ? 'dark:text-white dark:bg-red-500' : ''"
                          class="inline-block w-full rounded px-7 pt-3 pb-2.5 text-sm font-medium uppercase leading-normal
                   text-white shadow-[10px_10px_108px_16px_#FFFF66]
                    focus:outline-none focus:ring-0">{{ tab }}</button>
                </li>
              </ul>
            </div>
...
import {TabEnum} from "../Interfaces/TabInterface.ts";
